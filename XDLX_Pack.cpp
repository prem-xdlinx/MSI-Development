...
// This version contains bypass mode Packetization and frame formation
// This version contains the metadata EPH data writing functionality
#include <iostream>
#include <filesystem>
#include <fstream>
...
struct commonParameters{
    ...
    uint8_t MCID = 0;
    bool CCSDSProcessStatus = true; // Default to true, can be set from JSON
    int ephWriteCount;
    int sizeOfEphData;
    int coreTemperature;
...
bool fillStructureData(const json& config){
    try{
        ...
        // }
        param.CCSDSProcessStatus = config["CCSDSProcessStatus"].get<bool>();
        param.ephWriteCount = config["ephWriteCount"].get<int>();
        param.sizeOfEphData = config["sizeofEphData"].get<int>();
        param.coreTemperature = config["coreTemperature"].get<int>();
        param.sensorTemperature = config["sensorTemperature"].get<int>();
...
std::vector<uint8_t> packBandData(const std::vector<uint8_t>& data, const std::vector<uint8_t>& Meta, uint8_t apID) {
    ...
    int bitsPerByte = 8, bitdepth = 10, extraParamsLen = 20, bandHeight;
    ...
    int16_t paramVals[10] = {
        ...
        static_cast<int16_t>(param.coreTemperature),
        static_cast<int16_t>(param.sensorTemperature)
    };
    ...
    int MetaAddr = 0, MetaLen = 56 + EPHDataSize, k=0 ;
    size_t dataPos = 0, j=0;
    uint8_t seqFlag = 0b01;
    size_t extraParamsOffset = PRIMRY_HDR_LEN + 2 + MetaLen;
    int secondaryDataSize = MetaLen + EPHDataSize + param.mode.size() + extraParamsLen; // total secondary data size
    ...
    for (int i = 0, k = 0; i < totalPackets; i++) {
        ...
        if (metaCopyLen < MetaLen) {
            std::fill(header.begin() + PRIMRY_HDR_LEN + 2 + metaCopyLen,
                      ...
                      0x00);  // fill with 0x00
            std::cout<<"Metadata size( " <<metaCopyLen<<" ) is less than expected( "<<MetaLen<< " ). Padding with 0x00."<<std::endl; 
        }
        // Extra params
...
void processPacketFiles(const fs::path& baseDir, const std::string& baseName,
    ...
    // fs::path outStreamFile1 = baseDir / (baseName + ".stream1");
    // fs::path outStreamFile2 = baseDir / (baseName + ".stream2");
...
void processNonCCSDSFiles(const fs::path& baseDir, const std::string& baseName,
                  const std::vector<std::string>& suffixes, std::string& Metafile) {
    // fs::path outStreamFile1 = baseDir / (baseName + ".stream1");
    // fs::path outStreamFile2 = baseDir / (baseName + ".stream2");
    ...
    //Create a buffer to hold log data and meta data. Buffer size is param.width*384*10/8. for every start line represents its line number.
    std::vector<uint8_t> logMetaBuffer(param.width * 384 * 10 / 8, 0x00);
    std::cout<<"Initial buffer size allocated: "<<logMetaBuffer.size()<<" for log and meta data."<<std::endl;
    std::vector<uint8_t> Meta = readRawFile(Metafile);
    std::vector<uint8_t> logData = readRawFile(baseDir / (baseName + ".log"));
    if (!logData.empty()) {
        std::cout << "Log data size: " << logData.size() << std::endl;
    } else {
        std::cout << "[E102] No log data found in the file: " << baseDir / (baseName + ".log") << std::endl;
    }
    if (!Meta.empty()) {
        std::cout << "Meta data size: " << Meta.size() << std::endl;
    } else {
        std::cout << "[E103] No meta data found in the file: " << Metafile << std::endl;            
    }

    size_t lineSize = param.width * 10 / 8;
    size_t numLines = 384;
    size_t logDataOffset = 0;
    size_t metaDataOffset = 0;

    for (size_t line = 0; line < numLines; ++line) {
        size_t offset = line * lineSize;
        // Write 2-byte line number (little endian)
        logMetaBuffer[offset] = static_cast<uint8_t>(line & 0xFF);
        logMetaBuffer[offset + 1] = static_cast<uint8_t>((line >> 8) & 0xFF);

        if (line < 192) {
            // Log data lines
            size_t dataToCopy = std::min(lineSize - 2, logData.size() - logDataOffset);
            if (dataToCopy > 0) {
                std::memcpy(&logMetaBuffer[offset + 2], &logData[logDataOffset], dataToCopy);
                logDataOffset += dataToCopy;
            }
        } else if (line < 383) {
            // Meta data lines
            size_t dataToCopy = std::min(lineSize - 2, Meta.size() - metaDataOffset);
            if (dataToCopy > 0) {
                std::memcpy(&logMetaBuffer[offset + 2], &Meta[metaDataOffset], dataToCopy);
                metaDataOffset += dataToCopy;
            }
        } else if (line == 383) {
            // Last line: common parameters
            // Example: width, Height, totalFrames, binning, TDIMode, TDIStages, bandHeight, mode (as string)
            size_t paramOffset = offset + 2;
            auto writeLE16 = [](uint8_t* dest, int16_t value) {
                dest[0] = value & 0xFF;
                dest[1] = (value >> 8) & 0xFF;
            };
            writeLE16(&logMetaBuffer[paramOffset + 0],  static_cast<int16_t>(param.width));
            writeLE16(&logMetaBuffer[paramOffset + 2],  static_cast<int16_t>(param.Height));
            writeLE16(&logMetaBuffer[paramOffset + 4],  static_cast<int16_t>(param.totalFrames));
            writeLE16(&logMetaBuffer[paramOffset + 6],  static_cast<int16_t>(param.binning));
            writeLE16(&logMetaBuffer[paramOffset + 8],  static_cast<int16_t>(param.TDIMode));
            writeLE16(&logMetaBuffer[paramOffset + 10], static_cast<int16_t>(param.TDIStages));
            writeLE16(&logMetaBuffer[paramOffset + 12], static_cast<int16_t>(param.bandHeight));
            writeLE16(&logMetaBuffer[paramOffset + 14], static_cast<int16_t>(param.coreTemperature));
            writeLE16(&logMetaBuffer[paramOffset + 16], static_cast<int16_t>(param.sensorTemperature));
            // Write mode string (null-terminated or up to remaining space)
            size_t modeLen = std::min(param.mode.size(), lineSize - 2 - 14UL);
            std::memcpy(&logMetaBuffer[paramOffset + 18], param.mode.data(), modeLen);
            // The rest remains zero
        }
    }

    // Write the filled buffer to the output file
    outFileStream1.write(reinterpret_cast<const char*>(logMetaBuffer.data()), logMetaBuffer.size());
    outFileStream2.write(reinterpret_cast<const char*>(logMetaBuffer.data()), logMetaBuffer.size());
    std::vector<uint8_t> extraLineBuffer(param.width * 10 / 8, 0x00);
    for (const auto& suffix : suffixes) {
        fs::path filePath = baseDir / (baseName + suffix);
        if (!fs::exists(filePath)) continue;
        ...
        std::vector<uint8_t> data = readRawFile(filePath);
        if (data.empty()) {
            std::cout << "[E106] No data in the file: " << filePath << " Skipping." << std::endl;
            continue;
        }
        if(apid == 00 || apid == 02 || apid == 10 || apid == 12 || apid == 20 || apid == 22 || apid == 30 || apid == 32 || apid == 40 || apid == 50 || apid == 60 ){
            extraLineBuffer[0] = static_cast<uint8_t>(apid);
            std::cout<< "Writing data for APID: " << static_cast<int>(apid)<<"("<<suffix<<")" << " to stream1." << std::endl;
            outFileStream1.write(reinterpret_cast<const char*>(extraLineBuffer.data()), extraLineBuffer.size());
            outFileStream1.write(reinterpret_cast<const char*>(data.data()), data.size());
        }
        else if(apid == 01 || apid == 11 || apid == 21 || apid == 31 || apid == 41 || apid == 42 || apid == 51 || apid == 52 || apid == 61 || apid == 62){
            extraLineBuffer[0] = static_cast<uint8_t>(apid);
            std::cout<< "Writing data for APID: " << static_cast<int>(apid)<<"("<<suffix<<")" << " to stream2." << std::endl;
            outFileStream2.write(reinterpret_cast<const char*>(extraLineBuffer.data()), extraLineBuffer.size());
            outFileStream2.write(reinterpret_cast<const char*>(data.data()), data.size());
        }
    }
    // std::fill(logMetaBuffer.begin(), logMetaBuffer.end(), 0x00);
    // for (size_t line = 0; line < numLines; ++line) {
    //     size_t offset = line * lineSize;
    //     // Write 2-byte line number (little endian)
    //     logMetaBuffer[offset] = static_cast<uint8_t>(line & 0xFF);
    ...
    //         // Write mode string (null-terminated or up to remaining space)
    //         size_t modeLen = std::min(param.mode.size(), lineSize - 2 - 14UL);
    //         std::memcpy(&logMetaBuffer[paramOffset + 14], param.mode.data(), modeLen);
    //         // The rest remains zero
    //     } else if (line < 192) {
    //         // Meta data lines
    //         size_t dataToCopy = std::min(lineSize - 2, Meta.size() - metaDataOffset);
    ...
    outFileStream1.close();
    outFileStream2.close();
    std::cout << "Non-CCSDS files processed and written to: " << outStreamFile1 << " and " << outStreamFile2 << std::endl;
}     
...
void processFolder(const fs::path& folderPath, std::string& jsonFilePath, std::string& logFilePath) {
    ...
    if(!logFilePath.empty()){
        ...
        baseName = logPath.stem().string();
        baseDir = logPath.parent_path();
    }else if(!jsonFilePath.empty()){
        fs::path jsonPath(jsonFilePath);
        baseName = jsonPath.stem().string();
        baseDir = jsonPath.parent_path();
    }else{
        std::cout<<"[E102] No log or json file found in the folder: "<<folderPath<<std::endl;
        return;
    }
    ...
    if(param.CCSDSProcessStatus){
        std::cout<<"CCSDS Process Status is enabled. Processing files in folder: "<<folderPath<<std::endl;
        processPacketFiles(baseDir, baseName, suffixes, Metafile);
    }else{
        std::cout<<"CCSDS Process Status is disabled. Skipping packetization and frame formation for folder: "<<folderPath<<std::endl;
        processNonCCSDSFiles(baseDir, baseName, suffixes, Metafile);
    }
}
